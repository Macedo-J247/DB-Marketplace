...
SELECT p.id_produto AS id, p.nome_produto AS nome, p.descricao, p.preco, p.status, a.endpoint_url AS url, p.data_publicacao, p.data_atualizacao, p.tipo, p.categoria_id, p.desenvolvedor_id FROM produto p
JOIN api a ON a.produto_id = p.id_produto;

-- VIEW: Softwares com detalhes
CREATE OR REPLACE VIEW vw_softwares AS
SELECT p.id_produto AS id, p.nome_produto AS nome, p.descricao, p.preco, p.status, s.tipo_licenca, p.data_publicacao, p.data_atualizacao, p.tipo, p.categoria_id, p.desenvolvedor_id FROM produto p
...
SELECT p.id_produto, p.nome_produto, p.descricao, p.preco, p.tipo, p.status, p.data_publicacao, p.data_atualizacao, c.nome_categoria, d.nome_dev FROM produto p
...
SELECT v.id_versao, v.num_versao, v.data_lancamento, p.id_produto, p.nome_produto, d.nome_dev FROM versao v
...
SELECT a.id_assinatura, a.data_inicio, a.data_termino, a.status, u.nome_usuario, v.num_versao, p.nome_produto, tp.nome_tipo AS tipo_pagamento FROM assinatura a
...
SELECT par.id_parcela, par.valor, par.data_vencimento, par.data_pagamento, par.status, ass.id_assinatura, u.nome_usuario, p.nome_produto, v.num_versao FROM parcela par
...
SELECT s.id_suporte, s.tipo, s.descricao, s.status, s.data_suporte, u.nome_usuario, p.nome_produto, v.num_versao FROM suporte s
...
SELECT p.id_produto, p.nome_produto, COUNT(a.id_avaliacao) AS total_avaliacoes, ROUND(AVG(a.nota), 2) AS media_nota FROM avaliacao a
JOIN versao v ON a.versao_id = v.id_versao
JOIN produto p ON v.produto_id = p.id_produto
